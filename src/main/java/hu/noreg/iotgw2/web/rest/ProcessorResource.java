...
import hu.noreg.iotgw2.service.ProcessorQueryService;
import hu.noreg.iotgw2.service.ProcessorService;
...
/**
 * REST controller for managing {@link hu.noreg.iotgw2.domain.Processor}.
 */
@RestController
@RequestMapping("/api")
public class ProcessorResource {

    private final Logger log = LoggerFactory.getLogger(ProcessorResource.class);
    ...
    private final ProcessorQueryService processorQueryService;

    public ProcessorResource(
        ProcessorService processorService,
        ProcessorRepository processorRepository,
        ProcessorQueryService processorQueryService
    ) {
        this.processorService = processorService;
        this.processorRepository = processorRepository;
        this.processorQueryService = processorQueryService;
    }
    ...
    public ResponseEntity<ProcessorDTO> createProcessor(@Valid @RequestBody ProcessorDTO processorDTO) throws URISyntaxException {
        ...
        ProcessorDTO result = processorService.save(processorDTO);
        return ResponseEntity
            .created(new URI("/api/processors/" + result.getId()))
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/processors/{id}", consumes = "application/merge-patch+json")
    ...
    public ResponseEntity<List<ProcessorDTO>> getAllProcessors(ProcessorCriteria criteria, Pageable pageable) {
        log.debug("REST request to get Processors by criteria: {}", criteria);
        Page<ProcessorDTO> page = processorQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /processors/count} : count all the processors.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/processors/count")
    public ResponseEntity<Long> countProcessors(ProcessorCriteria criteria) {
        log.debug("REST request to count Processors by criteria: {}", criteria);
        return ResponseEntity.ok().body(processorQueryService.countByCriteria(criteria));
    }
    ...
    public ResponseEntity<ProcessorDTO> getProcessor(@PathVariable Long id) {
